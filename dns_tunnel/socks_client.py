...
def perform_socks5_handshake(
    server_ip: str, domain_name: str, socks_server_ip: str, socks_server_port: int
) -> None:
    """
...
class ClientHandler:
    ...
    def run(self):
        # listen server socket on 1080
        # add to reactor
        # listen for connect
        # on connect: accept, add send message to server greeting (with new session id),set state to waiting for greet response
        # add socket to rlist
        # wait for greet response, add to write a connect message, return to rlist, update state
        # wait for connect response, update state, add tcp src to rlist
        # TODO: Timeouts
        ...
        while True:
            ...
            for read_ready_client in read_ready_clients:
                ...
                for chunk in more_itertools.chuncked(data, DNSPacket.MAX_PAYLOAD):
            ...
            if proxy_socket in r_ready:
                ...
                for msg in msgs:
                    ...
                    proxy_socket.add_to_write_queue(
                        create_ack_message(
                            msg.header.session_id, msg.header.sequence_number
                        )
                    )
            ...
            write_ready_clients = [
                ready
                for ready in w_ready
                if ready in self._clients or ready == proxy_socket
            ]
            ...
            #### OLD VER
            # Reset read and write list after selectcycle
            # self._rlist = [server_socket]
            # self._wlist = []

            # if server_socket in r_ready:
            #     tcp_client_socket, _ = server_socket.accept()
            #     self._handle_accepted_tcp_client(tcp_client_socket)

            # for sock in r_ready:
            #     if sock in clients:
            #         # self.handle_client_by_state(sock)
            #         ...

            # # TODO: re-add client sockets as needed to read and write lists
    ...
    def _handle_accepted_tcp_client(self, tcp_client: socket):
        selectable_tcp_client = TCPClientSocket(
            tcp_client, TCPClientSocketState.SETTING_UP_TUNNEL
        )
        tunnel = TunnelSocket(
    ...
    def tunnel_to_tcp_client(self) -> dict[TunnelSocket, TCPClientSocket]:
        return {
            tunnel: tcp_client
            for tcp_client, tunnel in self.tcp_client_to_tunnel.items()
        }
